...
services:
  handbrake-server:
    ...
    ports:
      - 9999:9999
    environment:
      - HANDBRAKE_WEB_USERNAME=admin # change before exposing the server
      - HANDBRAKE_WEB_PASSWORD=change-this-password # change before exposing the server
      - HANDBRAKE_WORKER_SECRET=change-this-worker-secret # must match every worker
    volumes:
  ...
  handbrake-worker:
    ...
    environment:
      ...
      - HANDBRAKE_WORKER_SECRET=change-this-worker-secret # must match the server
    volumes:
      - /path/to/your/media:/video # ensure this path is the same across all containers
    depends_on:
      - handbrake-server
